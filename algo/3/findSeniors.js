...
function findSeniors(persons) {
  ...
  const devsWebSeniors = persons.filter(person =>
    person.job === 'web dev' && person.experience >= 5
  );

  const dataAnalystsSeniors = persons.filter(person =>
    person.job === 'data analyst' && person.experience >= 5
  );

  return [devsWebSeniors, dataAnalystsSeniors];
}
