...
function findSeniors(persons) {
  const seniorWebDevs = persons.filter(person => person.experience >= 5 && person.job === 'web dev');
  const seniorDataAnalysts = persons.filter(person => person.experience >= 5 && person.job === 'data analyst');

  return [seniorWebDevs, seniorDataAnalysts];
}
